...
// Lets start this thing!
	class bot {
		...
		private $user = "";
		private $realname = "";
		public function bot($server="irc.synirc.net", $port = 6667, $nick = "MCABot", $user = "MCABot", $realname = "MCABot", $startchan = "#MineBot") {
	...
	class command {
		...
		public function command_me($user, $channel, $args) {
			$ACTION = chr(1);
			$msg = implode(" ",$args);
			$this->bot->say_message($channel, $ACTION."ACTION ".$msg." ".$ACTION);
		}
		...
		public function command_eval($user, $channel, $args) {
			if ($user == "SinZ!~SinZ@mca.uk.to") {
				$msg = implode(" ", $args);
				$this->bot->say_message($channel, eval("return ".substr($msg,0)));
			}
			else {
				$this->bot->say_message($channel, "nty");
			}
		}
	...
	class fun_commands {
		...
		public function command_slap($user, $channel, $args) {
			$ACTION = chr(1);
			$this->bot->say_message($channel, $ACTION."ACTION slaps ".$args[0]." with a large frozen tuna!".$ACTION);
		}
		public function command_rickroll($user, $channel, $args) {
			...
			else if ($target == "mcabot") {
				$this->bot->say_message($channel, "...dont rickroll me pl0x");
			}
			else {
		...
		public function command_test($user, $channel, $args) {
			require_once("/plugins/ranks/plugin.php");
			$rank = bot_ranks($user, $channel, $this->server);
			if ($rank == true) {
				$this->bot->say_message($channel, "You can use me!");
			}
			else if ($rank == false) {
				$this->bot->say_message($channel, "You dont have permissions to use this command!");
			}
			else {
				//meh
			}
	...
	$bot->plugin_register(new fun_commands());
	$bot->plugin_register(new minecraft_commands());
	$bot->start();
?>